...
export class EcowittAccessory
{
  ...
  {
    service.setCharacteristic(this.platform.Characteristic.StatusActive, active);
  }

  updateStatusActive(service: Service, active: boolean)
  {
    service.updateCharacteristic(this.platform.Characteristic.StatusActive, active);
  }
  ...
  addOccupancySensor(name: string)
  {
    const occupancySensor = this.accessory.getService(name) ||
      this.accessory.addService( this.platform.Service.OccupancySensor, name, this.serviceUuid(name));
  ...
  {
    service.updateCharacteristic(this.platform.Characteristic.OccupancyDetected, occupancyDetected ?
  ...
  addMotionSensor(name: string)
  {
    const motionSensor = this.accessory.getService(name) ||
      this.accessory.addService(this.platform.Service.MotionSensor, name, this.serviceUuid(name));
  ...
  {
    service.updateCharacteristic(this.platform.Characteristic.MotionDetected, motionDetected);
  }
  ...
  addBattery(name: string, chargeable = false)
  {
    ...
    battery.setCharacteristic(this.platform.Characteristic.StatusLowBattery,
  ...
  {
    service.updateCharacteristic(this.platform.Characteristic.CurrentTemperature, Utils.toCelcius(tempf));
  }
  ...
  {
    service.updateCharacteristic(this.platform.Characteristic.CurrentRelativeHumidity, parseFloat(humidity));
  }
