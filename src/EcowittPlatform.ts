...
export class EcowittPlatform implements DynamicPlatformPlugin
{
  ...
  onDataReport(dataReport)
  {
    ...
    this.log.debug('Data report:', JSON.stringify(dataReport, undefined, 2));
  ...
  registerAccessories(dataReport)
  {
    ...
    {
      const sensorId = this.config.mac + '-' + sensor.type + (sensor.channel > 0 ? '-' + sensor.channel.toString() : '');
      ...
      if (existingAccessory)
      {
        ...
        this.CreateAccesory(sensor, existingAccessory);
      }
      else
      {
  ...
  //----------------------------------------------------------------------------
  CreateAccesory(sensor, accessory)
  {
    switch (sensor.type)
    {
      case 'GW1000':
